...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    if (enable_tracking) {
      ...
      apiFetch<{ total_sessions: number }>("/tracking/admin/analytics/sessions")
        .then((d) =>
          setStats((s) => ({ ...s, sessions: d.total_sessions || 0 })),
        )
        .catch(() => {});
    }
    ...
    fetch("/api/health")
      ...
      .then((data: { status: string; services: Record<string, string> }) => {
        const result: ServiceHealth[] = [
          ...
          {
            name: "redis",
            status: data.services?.redis === "ok" ? "ok" : "error",
          },
        ];
        setServices(result);
      })
  ...
  return (
    <div>
      ...
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 ${enable_tracking ? "lg:grid-cols-4" : "lg:grid-cols-2"} gap-4 mb-8`}
      >
        {enable_tracking && (
          <>
        ...
        <div className="glass rounded-xl p-5">
          ...
          <p
            className={`text-2xl font-bold ${stats.pendingDeletions > 0 ? "text-accent-pink" : "text-text-primary"}`}
          >
          ...
          {checking ? (
            ...
            <p className="text-sm">
              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-500/20 text-red-300 border border-red-500/30">
                {services.length - healthyCount} service
                {services.length - healthyCount !== 1 ? "s" : ""} degraded
              </span>
            </p>
          )}
          {statusExpanded && (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            ...
            <a
              key={link.href}
              href={link.href}
              className="glass rounded-lg p-3 text-sm text-text-secondary hover:text-accent-pink hover:border-accent-pink/20 transition-all text-center"
            >
              {link.label}
            </a>
          ))}
