...
export default function MerchantDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await apiFetch<MerchantStatus>(
          "/payments/merchants/status",
        );
        setStatus(data);
      } catch {
        setError("not_found");
  ...
  if (user?.role !== "merchant") {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        ...
        <p className="text-text-secondary mb-6">
          You need a merchant account to access this page.
        </p>
  ...
  if (error === "not_found" || !status) {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        ...
        <p className="text-text-secondary mb-6">
          Your merchant account is ready, but you haven&apos;t connected Stripe
          yet. Complete onboarding to start receiving payments.
        </p>
  ...
  async function openStripeDashboard() {
    ...
    try {
      const data = await apiFetch<{ dashboard_url: string }>(
        "/payments/merchants/dashboard-link",
      );
  ...
  const isPending =
    ...
    (!status.charges_enabled && status.status !== "rejected");

  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="font-serif text-2xl font-bold gradient-text mb-6">
        Merchant Dashboard
      </h1>
      ...
      <div className="glass rounded-2xl p-6 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary">
              {status.business_name || "Not set"}
            </p>
          </div>
          <div>
            <p className="text-xs text-text-muted mb-1">Charges</p>
            <p
              className={`text-sm font-medium ${status.charges_enabled ? "text-accent-green" : "text-text-muted"}`}
            >
          ...
          <div>
            ...
            <p
              className={`text-sm font-medium ${status.payouts_enabled ? "text-accent-green" : "text-text-muted"}`}
            >
        ...
        {isPending && (
          <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-sm text-yellow-400 mb-4">
            Your account is being verified by Stripe. This usually takes a few
            minutes. If verification is incomplete, click below to continue
            onboarding.
          </div>
        )}
