...
const STORAGE_KEY = "seo-advisor-business-context";
...
export default function SEOAdvisorPanel({
  path,
  onRuleClick,
}: SEOAdvisorPanelProps) {
  ...
  async function handleAnalyze() {
    ...
    try {
      const res = await apiFetch<AdvisorResponse>(
        `/seo/admin/seo/advisor/${encodeURIComponent(path)}`,
        {
          method: "POST",
          body: JSON.stringify({
    ...
    <div className="glass rounded-xl p-6">
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          className="w-full px-3 py-2 bg-glass-bg border border-glass-border rounded-lg text-xs text-text-primary placeholder-text-muted focus:border-accent-blue focus:outline-none resize-none"
          placeholder="e.g., Rank for 'Italian leather shoes', prepare for product launch, general audit..."
        />
      </div>
      ...
      {loading && (
        <div className="py-6 text-center space-y-2">
          ...
          <p className="text-xs text-text-muted">
            Analyzing page SEO{intent ? " for your goal" : ""}...
          </p>
          <p className="text-[10px] text-text-muted">
            This may take up to 60 seconds if researching keywords.
          </p>
        </div>
      )}
                ...
                >
                  ...
                  {s.rule_id && (
                    <span
                      ...
                      title="Highlight in score breakdown & HTML viewer"
                    >
                      {s.rule_id}
                    </span>
                  )}
