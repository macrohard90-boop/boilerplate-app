    ...
    <header className="fixed top-0 left-0 right-0 z-50 glass">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 text-text-primary font-serif text-xl font-bold">
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 glass rounded-lg py-2 z-50">
                      <div className="px-4 py-2 border-b border-glass-border">
                        <p className="text-sm text-text-primary font-medium">{user?.first_name} {user?.last_name}</p>
                        <p className="text-xs text-text-muted">{user?.email}</p>
                        <span className={`mt-1 inline-block text-xs px-2 py-0.5 rounded-full ${
                          user?.role === "admin" ? "bg-accent-pink/10 text-accent-pink" :
                          user?.role === "merchant" ? "bg-accent-blue/10 text-accent-blue" :
                          "bg-accent-green/10 text-accent-green"
                        }`}>
                          {user?.role}
                        </span>
                      </div>
                      <Link href="/dashboard" className="block px-4 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-glass-hover transition-colors" onClick={() => setUserMenuOpen(false)}>
                        Dashboard
                      </Link>
                      <Link href="/dashboard/orders" className="block px-4 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-glass-hover transition-colors" onClick={() => setUserMenuOpen(false)}>
                        Orders
                      </Link>
