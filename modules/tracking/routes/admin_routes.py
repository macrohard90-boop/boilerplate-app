...
async def get_session_stats(
    ...
    return SessionStats(
        ...
        date_to=lbl_to,
        by_day=[
            SessionDayStat(date=str(r["date"]), sessions=r["sessions"]) for r in by_day
        ],
    )
...
async def get_event_stats(
    ...
    total = (
        (
            await db.execute(
                text(
                    f"SELECT COUNT(*) AS total_events FROM analytics.events e {where}"
                ),
                params,
            )
        )
...
async def get_dashboard_summary(
    ...
    async def _kpi_for_range(start: datetime, end: datetime) -> dict[str, float]:
        ...
        pv = (
            (
                await db.execute(
                    text(
                        "SELECT COUNT(*) AS total_views, "
                        "COUNT(DISTINCT COALESCE(CAST(pv.user_id AS text), "
                        "pv.session_id)) AS unique_visitors "
                        "FROM analytics.page_views pv "
                        "WHERE pv.created_at >= :ts_from "
                        f"AND pv.created_at < :ts_end{bot_pv}"
                    ),
                    params,
                )
            )
            ...
            (
                await db.execute(
                    text(
                        ...
                        "FROM analytics.analytics_sessions s "
                        "WHERE s.started_at >= :ts_from "
                        f"AND s.started_at < :ts_end{bot_s}"
                    ),
                    params,
                )
            )
        ...
        bounce = (
            (
                await db.execute(
                    text(
                        "SELECT COUNT(*) FILTER (WHERE s.page_count = 1) "
                        "AS bounced, COUNT(*) AS total "
                        "FROM analytics.analytics_sessions s "
                        "WHERE s.started_at >= :ts_from "
                        f"AND s.started_at < :ts_end{bot_s}"
                    ),
            ...
            (
                await db.execute(
                    text(
                        ...
                        ") "
                        "SELECT "
                        "  COUNT(DISTINCT CASE WHEN uf.first_visit >= :ts_from "
                        "AND uf.first_visit < :ts_end "
                        "THEN uf.user_id END) AS new_visitors, "
                        "  COUNT(DISTINCT CASE WHEN uf.first_visit < :ts_from "
                        "THEN pv.user_id END) AS returning_visitors "
                        "FROM user_firsts uf "
                        "LEFT JOIN analytics.page_views pv "
                        "ON pv.user_id = uf.user_id "
                        "  AND pv.created_at >= :ts_from "
                        f"AND pv.created_at < :ts_end{bot_pv.replace('pv.', 'pv.')}"
                    ),
                    params,
                )
            )
        ...
        active = (
            (
                await db.execute(
                    text(
                        "SELECT COALESCE(SUM(pv.duration_ms), 0) / 1000.0 "
                        "AS total_active_sec "
                        "FROM analytics.page_views pv "
                        "WHERE pv.created_at >= :ts_from "
                        f"AND pv.created_at < :ts_end{bot_pv}"
        ...
        avg_active = (
            float(active["total_active_sec"]) / total_sessions
            if total_sessions > 0
            else 0
        )
...
async def get_pageview_timeseries(
    ...
    trunc_tpl = (
        f"TO_TIMESTAMP(FLOOR(EXTRACT(EPOCH FROM {{a}}.{{c}}) / {bsec}) * {bsec})"
    )
        ...
        (
            await db.execute(
                text(
                    ...
                    "  FROM analytics.page_views pv "
                    "  WHERE pv.created_at >= :ts_from "
                    f"  AND pv.created_at < :ts_end{bot} "
                    ...
                    f"TO_TIMESTAMP(EXTRACT(EPOCH FROM ps.bucket) + {bsec})) "
    ...
    return PageviewTimeSeries(
        points=[
            TimeseriesPoint(
                ...
                engagement_pct=(
                    float(r["engagement_pct"])
                    if r["engagement_pct"] is not None
                    else None
                ),
            )
            for r in rows
...
async def get_active_sessions(
    ...
    for r in rows:
        duration = (
            int((now - r["started_at"].replace(tzinfo=timezone.utc)).total_seconds())
            if r["started_at"]
            else 0
        )
        sid = r["session_id"]
    ...
    return PagesEngagementResponse(
        pages=[
            PageEngagement(
                ...
                unique_visitors=r["unique_visitors"],
                avg_duration_ms=(
                    float(r["avg_duration_ms"]) if r["avg_duration_ms"] else None
                ),
...
def _compute_engagement(
    total_sessions: int, avg_duration: float, last_active: datetime | None
) -> str:
    now = datetime.now(timezone.utc)
    recent = (
        last_active and (now - last_active.replace(tzinfo=timezone.utc)).days <= 7
        if last_active
        else False
    )
...
async def get_enriched_users(
    ...
    exclude_bots: bool = Query(True),
    sort_by: str = Query(
        "last_active", regex="^(last_active|pageviews|sessions|active_time|engagement)$"
    ),
    ...
    if segment == "active_today":
        ...
        segment_clause = (
            "HAVING MAX(pv.created_at) < :seg_start OR MAX(pv.created_at) IS NULL"
        )
        params["seg_start"] = now - timedelta(days=7)
        ...
        (
            await db.execute(
                text(
                    ...
                    "(s.ended_at - s.started_at))), 0) "
                    "AS avg_session_duration_sec, "
                    "(SELECT pv2.path FROM analytics.page_views pv2 "
                    "WHERE pv2.user_id = u.id "
                    "  GROUP BY pv2.path ORDER BY COUNT(*) DESC LIMIT 1) "
                    "AS top_page, "
                    "(SELECT COALESCE(SUM(pv3.duration_ms), 0) / 1000.0 "
                    "FROM analytics.page_views pv3 "
                    "  WHERE pv3.user_id = u.id "
                    "AND pv3.created_at >= :ts_from "
                    f"AND pv3.created_at < :ts_end"
                    f"{bot.replace('pv.', 'pv3.')}) "
                    "AS total_active_sec, "
                    "(SELECT COALESCE(SUM(EXTRACT(EPOCH FROM "
                    "(s2.ended_at - s2.started_at))), 0) "
                    "FROM analytics.analytics_sessions s2 "
                    "  WHERE s2.user_id = u.id "
                    "AND s2.started_at >= :ts_from "
                    "AND s2.started_at < :ts_end) AS total_wall_sec, "
                    "CASE WHEN (SELECT COALESCE(SUM(EXTRACT(EPOCH FROM "
                    "(s3.ended_at - s3.started_at))), 0) "
                    "FROM analytics.analytics_sessions s3 "
                    "  WHERE s3.user_id = u.id "
                    "AND s3.started_at >= :ts_from "
                    "AND s3.started_at < :ts_end) > 0 "
                    "THEN LEAST("
                    "(SELECT COALESCE(SUM(pv4.duration_ms), 0) / 1000.0 "
                    "FROM analytics.page_views pv4 "
                    "  WHERE pv4.user_id = u.id "
                    "AND pv4.created_at >= :ts_from "
                    f"AND pv4.created_at < :ts_end"
                    f"{bot.replace('pv.', 'pv4.')}) / "
                    "(SELECT SUM(EXTRACT(EPOCH FROM "
                    "(s4.ended_at - s4.started_at))) "
                    "FROM analytics.analytics_sessions s4 "
                    "  WHERE s4.user_id = u.id "
                    "AND s4.started_at >= :ts_from "
                    "AND s4.started_at < :ts_end) * 100, 100) "
                    ...
                    "AND pv.created_at >= :ts_from "
                    f"AND pv.created_at < :ts_end{bot} "
                    "LEFT JOIN analytics.analytics_sessions s "
                    "ON u.id = s.user_id "
                    "AND s.started_at >= :ts_from "
                    "AND s.started_at < :ts_end "
    ...
    for r in rows:
        ...
        eng_pct = (
            round(float(r["engagement_pct"]), 1)
            if r["engagement_pct"] is not None
            else None
        )
        users.append(
...
async def get_user_activity(
        ...
        (
            await db.execute(
                text(
                    ...
                    "WHERE user_id = CAST(:uid AS uuid)) AS pv, "
                    "(SELECT COUNT(*) FROM analytics.analytics_sessions "
                    "WHERE user_id = CAST(:uid AS uuid)) AS sess, "
                    "(SELECT MIN(created_at) FROM analytics.page_views "
                    "WHERE user_id = CAST(:uid AS uuid)) AS first_visit, "
                    "(SELECT MAX(created_at) FROM analytics.page_views "
                    "WHERE user_id = CAST(:uid AS uuid)) AS last_visit, "
                    "(SELECT COALESCE(SUM(duration_ms), 0) / 1000.0 "
                    "FROM analytics.page_views "
                    "WHERE user_id = CAST(:uid AS uuid)) "
                    "AS total_time_sec, "
                    ...
                    "  FROM analytics.analytics_sessions "
                    "WHERE user_id = CAST(:uid AS uuid) "
                    "AND ended_at IS NOT NULL) AS avg_sess_dur, "
                    "(SELECT COALESCE(SUM(EXTRACT(EPOCH FROM "
                    "(ended_at - started_at))), 0) "
                    "  FROM analytics.analytics_sessions "
                    "WHERE user_id = CAST(:uid AS uuid) "
                    "AND ended_at IS NOT NULL) "
                    "AS total_session_time_sec, "
    ...
    for sr in sess_rows:
        if sr["session_id"] in live_session_ids:
            live_wall = (
                ...
                - sr["started_at"].replace(tzinfo=timezone.utc)
            ).total_seconds()
    ...
    avg_eng_all = (
        round(total_active_sec / total_session_sec * 100, 1)
        if total_session_sec > 0
        else 0.0
    )
    ...
    return UserActivity(
        ...
        avg_active_per_session_sec=(
            round(float(totals["avg_active_per_session"]), 1)
            if totals["avg_active_per_session"]
            else 0.0
        ),
        avg_engagement_all_pct=avg_eng_all,
        avg_engagement_live_pct=avg_eng_live,
        top_pages=[
            UserTopPage(path=r["path"], views=r["views"]) for r in top_pages_rows
        ],
