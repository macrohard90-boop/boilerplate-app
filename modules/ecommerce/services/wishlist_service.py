...
async def list_wishlists(db: AsyncSession, user_id: str) -> list[dict[str, Any]]:
    ...
    rows = (
        await db.execute(
            text("SELECT * FROM ecommerce.wishlists WHERE user_id = :uid ORDER BY created_at"),
            {"uid": user_id},
        )
    ).mappings().all()
...
async def add_item(
    db: AsyncSession, user_id: str, wishlist_id: str | None,
    product_id: str, variant_id: str | None,
) -> dict[str, Any]:
    ...
    if not wishlist_id:
        ...
        wl = (
            await db.execute(
                text("SELECT id FROM ecommerce.wishlists WHERE id = :wid AND user_id = :uid"),
...
async def remove_item(
    db: AsyncSession, user_id: str, wishlist_id: str, product_id: str,
) -> None:
    # Verify ownership
    wl = (
        await db.execute(
...
async def delete_wishlist(db: AsyncSession, user_id: str, wishlist_id: str) -> None:
    result = await db.execute(
        text("DELETE FROM ecommerce.wishlists WHERE id = :wid AND user_id = :uid AND is_default = FALSE"),
        {"wid": wishlist_id, "uid": user_id},
    )
    if result.rowcount == 0:
...
async def _get_wishlist_items(db: AsyncSession, wishlist_id: str) -> list[dict[str, Any]]:
    rows = (
        await db.execute(
            text(
