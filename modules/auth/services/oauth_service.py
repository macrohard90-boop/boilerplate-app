...
async def create_state_token(redis: Redis) -> str:
...
async def validate_state_token(redis: Redis, state: str) -> bool:
...
async def find_or_create_user(
    ...
    await db.commit()
    return {**dict(new_user), "role": "customer"}  # type: ignore[union-attr]
